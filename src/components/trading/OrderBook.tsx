...
export const OrderBook: React.FC<OrderBookProps> = ({ orders, onRefresh }) => {
  ...
  const historyOrders = useMemo(() => 
    orders.filter(o => o.status !== 'ACTIVE' && o.status !== 'PENDING').slice(0, 20),
    [orders]
  );
      ...
      {displayOrders.length === 0 ? (
        ...
        <div className="space-y-2 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-1 sm:pr-2 scrollbar-thin">
          {displayOrders.map((order) => {
            const remaining = remainingTimes[order.id] || 0;
            ...
            return (
              <div
                key={order.id}
                className="bg-primary-900/30 border border-primary-800 hover:border-primary-700 rounded-lg p-3 sm:p-4 transition-all group"
              >
                {/* Header - Responsive */}
                ...
                {/* Progress Bar for Active Orders - Responsive */}
                {order.status === 'ACTIVE' && (
                ...
                {/* Exit Price for Completed Orders - Responsive */}
